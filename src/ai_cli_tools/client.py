...
from ai_cli_tools.exceptions import AIModelNotFoundError, AIResponseError, AITimeoutError
...
"""
        ...
        try:
            ...
            raise AIModelNotFoundError(
                f"{ai_model.name} CLI를 찾을 수 없습니다. 명령어: {' '.join(ai_model.command)}"
            )
        except subprocess.TimeoutExpired:
            raise AITimeoutError(f"{ai_model.name} 응답 타임아웃 ({self.timeout}초 초과)")
    ...
    def call_ai_with_retry(
        self, prompt: str, ai_model: AIModel, agents: List[str] = None, max_retries: int = 3
    ) -> str:
        """재시도 로직을 포함한 AI 호출
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(
                        f"⚠️  {ai_model.name} 타임아웃 발생, 재시도 중... ({attempt + 1}/{max_retries})"
                    )
                ...
                if attempt < max_retries - 1:
                    ...
                    print(
                        f"⚠️  {ai_model.name} 오류 발생, 재시도 중... ({attempt + 1}/{max_retries})"
                    )
